...
const completeExample = "career_scout.json"
...
func TestTheExamplesTrackTheEngine(t *testing.T) {
	fixture := filepath.Join("..", "chargen", "testdata", completeExample)

	if canonical(t, "character.complete.json") != canonical(t, fixture) {
		t.Errorf("character.complete.json is no longer %s; re-copy it, because task goldens does not", fixture)
	}
...
func TestAKeywordBesideARefStillApplies(t *testing.T) {
	...
	if problems := c.check(c.root, readRecord(t, "character.minimal.json"), "record"); len(problems) == 0 {
...
func TestEverySchemaPropertyIsExercised(t *testing.T) {
	...
	seen := keysAcross(t, append(records, "character.minimal.json", "character.complete.json"))
