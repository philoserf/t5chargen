...
| Psi deferred, never rolled                          | non-goals     | `RollCharacteristics` doc                                | golden stream                 | covered (v1 excludes psionics; the non-goal names Psionics only, and said nothing about Sanity — see the p. 52 row below) |
| Sanity is not generated during character generation | p. 52         | `recordSanityMod` records a modifier; no value is rolled | `TestScoutSanityModifier`     | interpretation I-47 — chart A p. 89 never mentions San; p. 52 is the governing page                                       |
