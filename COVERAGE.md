...
| "Dying During Character Generation": "all efforts in this particular character creation process are lost" | p. 69                     | `Character.Dead` set; the record is still returned | golden seed 305 (Soldier)                                      | deferred — the engine returns the dead character rather than discarding the run; whether "lost" governs the tool's output or only play at the table is an open reading |
