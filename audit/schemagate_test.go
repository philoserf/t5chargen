...
const (
	...
	// completeExample is a golden fixture rather than a copy of one. A
	// copy in docs/ is a second record that `task goldens` cannot reach,
	// and it needed a gate of its own to notice when it drifted; the
	// fixture is the engine's own output and moves when the engine does.
	completeExample = "../chargen/testdata/career_scout.json"
)
...
// TestTheExamplesTrackTheEngine keeps the two examples honest about
// provenance: an example advertising a schema_version the engine no
// longer stamps documents a contract that has moved.
...
func TestEverySchemaPropertyIsExercised(t *testing.T) {
	...
	seen := keysAcross(t, append(records, minimalExample))
